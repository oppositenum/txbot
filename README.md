...
账号总览右上角提供 **导出** 和 **导入**。导出文件包含账号备注、账号密码、Cookie、独立代理和全部任务配置，不包含日志、金币等级或任务排期等临时状态。迁移包是明文敏感文件，只能通过可信渠道传输，导入完成后应及时删除不再需要的副本。

目标服务器导入时默认暂停新增账号并跳过重复账号；旧服务器已经停止后，可勾选“沿用导出时的启用状态”直接恢复托管。需要以迁移包替换目标服务器已有同账号配置时，再显式开启“覆盖目标服务器中的同账号”。同一批账号不要同时在两台服务器运行。
...
- **农场 job**：收割→耕→种→护理后，解析每块地的**成熟倒计时**，把下次唤醒设在最近成熟时刻；进入最后一分钟后每5秒尝试抢收，单轮持续最多90秒，临界网络错误也会快速重试。另设一个**兜底上限**（默认5分钟必查一次），所以别人帮你浇水加速成熟、或作物被偷，最多延迟这个上限就能发现。上限可在「任务配置」调。
- **每日 job**：每天 `dailyStartHour`（默认0点）之后，在错峰范围内随机一个时刻执行所有签到/每日领取；按自然日去重，一天只跑一次。服务中途启动若当天没做过会补做。
- **聊天室抢积分**：按配置的小时/分钟开始（支持 `00:00`），之后持续轮询，只有网站明确返回当日上限才停止到次日。没活动、被别人抢先或网络错误都不会被记为完成；关闭账号或抢积分开关可停止。管理页显示实际成功次数、积分和当前状态。旧 `grabWindowMin` 不再限制总时长，`grabPollSec` 是每轮完成后的等待秒数（最低 1 秒；新账号默认 1 秒，已有配置保留）。
...
抢积分使用独立的每账号轮询任务，不占普通任务的并发槽，也不受单个普通任务 20 分钟看门狗限制。每个账号同一时间只有一轮抢取；成功和当日上限立即保存，普通进度每 30 秒保存一次。旧版本的 `lastGrab` 只代表曾运行过，升级后不会据此跳过当天抢取；新的完成依据是网站确认的 `grabStatus.state=full`。
